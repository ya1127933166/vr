<template>
  <view class="find-page">
    <!-- <view v-if="isShowBar" class="statusBar" :style="'width: 750rpx;height:' + (headerHeight - 0 + 10) + 'px;'"></view> -->
    <view class="scrollBar"
          :style="'padding-top:' + (isShowBar?boundInfo.top:'0') + 'px;height:' + (isShowBar?headerHeight-0+boundInfo.top:'0') + 'px;'">
      <view @tap="toLocation"
            class="barLeft"
      ...
      <view class="home-head-group">
        ...
        <!-- 	<view class="menuBox">
					...
					<view class="menuItem menuBorder">
						<view class="count">10</view>
						<view class="text">本月已租</view>
					</view>
					<view class="menuItem">
						<view class="count">10</view>
						<view class="text">总房源</view>
					</view>
				</view> -->
        <view class="tab"
              v-if="needTabs && tabList && tabList.length">
          <view class="tabTitle"
    ...
    <scroll-view :scroll-y="isShowBar"
                 ...
                 :style="'height:'+(isScroll?'calc(' + scrollHeight + 'px - ' + hackHeight + 'rpx)':'calc('+hackScroll+'px - 100rpx)')+';box-sizing: border-box;'"
                 lower-threshold='75'
                 @scrolltolower='lower'>
      <block v-if="houseList.length&&houseList.length>0">
        ...
        <view v-if="isShow!=true"
              style="width:750rpx;height:100rpx;text-align:center;font-size:30rpx;color:#cccccc;line-height:100rpx">已加载全部</view>
      </block>
...
export default {
  ...
  data () {
    return {
      ...
      tabList: ['二手房', '租房'],
      tag: '',
      indexCurry: this.defaultIndex || 0,
      //请求页码
      currentPage: 0,
      totalPage: 0,
      pageSize: 20,
  ...
  mounted () {
    // console.warn('mounted',{storeCity: this.storeCity}, this.locationInfo)
    if (this.locationInfo && this.locationInfo.id) {
      this.storeCity = this.locationInfo.id
    }
  },
  computed: {
    ...mapState({
  ...
  watch: {
    ...
    houseList: {
      handler (newValue, oldValue) {
        let that = this
        const sys = that.$$globalData.systemInfo
        this.checkScroll({ sys, that, listLength: newValue.length || 0 })
      },
      deep: true,
      immediate: true
    }
  ...
  methods: {
    ...
    showHeaderCallBack (e) {
      // console.warn('showHeaderCallBack', e)
      this.isShowBar = e
    },
    //下拉菜单点击事件
    downMenuBarClick (e) {
      // console.log('选择', e, this.indexCurry);
    ...
    componentShow () {
      ...
      const sys = this.$$globalData.systemInfo
      const { id: cityId } = this.locationInfo || {};
      // if (this.storeCity && cityId && this.storeCity !== cityId) {
      // }
      console.warn({ storeCity: this.storeCity, cityId })
      let isSameCity = true;
      if (this.storeCity && cityId && (this.storeCity !== cityId)) {
        isSameCity = false;
        this.storeCity = cityId
      }
      if (!this.$utils.matchPage(this.pageKey) || !isSameCity) {
        uni.pageScrollTo({
          scrollTop: 0
        })
        this._cleanData();
      ...
      this.$utils.storeCurrentPage(this.pageKey)
      if (this.isPhone || !isSameCity) {
        if (+this.indexCurry === 0) {
          this.OldHouseList();
        } else if (+this.indexCurry === 1) {
          this.RentHouseList()
      ...
      this.checkScroll({ sys, that: this, listLength: this.houseList.length })
    },
    componentHide () {
      // this._cleanData(false)
    },
    _cleanData (cleanSelected = true) {
      if (cleanSelected) {
    ...
    clickAddHouseCallBack (e) {
      ...
      //初始化添加房源页面配置
      this.setPageInfo({ type: '添加房源', title: [`${title}`, '添加房源'], processList: [{ text: '房屋概况', status: true }, { text: '设置价格', status: false }, { text: '完成', status: false }], len: [0, 50] })
    ...
    clickHouseItem (item) {
      ...
      // console.log('clickHouseItem', item, this.isContract)
      //二手房请详页面
      if (+this.indexCurry === 0) {
        obj = {
          id: item.houseSale.id || '',
          name: item.house.name || ''
        }
        const jumpPath = isContract ? '../../../pages/ownerContract/ownerContractRecorder' : '../../subpackComm/pages/viewHousing/viewHousing'
        this.$$globalData.jumpPath = jumpPath
        this.checkLogin(() => {
          uni.navigateTo({
            url: `${jumpPath}?type=0&houseId=${JSON.stringify(obj)}`,
      ...
      if (+this.indexCurry === 1) {
        ...
        if (isContract) {
          this.$store.state.findPageClickItem = item
        }
        const jumpPath = isContract ? '../../../pages/ownerContract/ownerContractRecorder' : '../../subpackComm/pages/viewHousing/viewHousing1'
        ...
        this.checkLogin(() => {
          uni.navigateTo({
            url: `${jumpPath}?type=0&houseId=${JSON.stringify(obj)}`,
            complete: (e) => {
              // console.log('navigateTo', e, jumpPath)
              uni.hideLoading()
            }
          })
        })
    ...
    checkScroll ({ sys, that = this, listLength = 0 }) {
      if (listLength) {
        let view = uni.createSelectorQuery().in(that).select(".houseList");
        //计算是否禁止滚动
        try {
          that.$nextTick(() => {
            view.boundingClientRect(data => {
              const canScroll = data && data.height && (((listLength + 1) * data.height) > (+sys.windowHeight - 10) - data.top)
              that.isScroll = canScroll
            }).exec();
          })
        } catch (error) {
        }
      } else if (that.needFilter) {
        //计算无数据时scroll-view高度
    ...
    chooseTab (item, index) {
      this.$utils.debounce(() => {
        ...
        this.checkLogin(() => {
          ...
          // console.warn('this.checkLogin', index)
          // 0二手房 1租房
          if (index == 0) {
            this.OldHouseList()
            this.menuBar = [
    ...
    //二手房查询列表
    async OldHouseList () {
      // console.log('OldHouseList', this.roomNum)
      this.isShow = true
      let temp = {
        cityId: this.locationInfo.id,
      ...
      if (temp.currentPage <= this.totalPage + 1) {
        try {
          let res = await this.request({
            url: 'house/sale/list',
            type: 'post',
    ...
    async RentHouseList () {
      try {
        if (this.isContract) {
          ...
          let temp = {
            type: 'allrent', // 二手房 sale  整租 allrent  合租 sharerent
            xiaoquId: selectedHousing.id //
          }
          ...
          let res = await this.request({
            ...
            url: `house/${temp.type}/xiaoqu/${selectedHousing.id}`,
            type: 'get',
            param: {
              currentPage: this.currentPage + 1,
              src: 'contract'
          ...
          // console.log('is contract', this.isContract, res)
          if (res[1].data.state == 200) {
            ...
            for (const item of body) {
              ...
              const { houseId, title, xqName, tags, houseUnit, orientation, size, price, payStr, houseImage, pledge, rentId } = item
          ...
          // console.log('是否请求',this.currentPage,this.totalPage)
          if (this.currentPage <= this.totalPage) {
            ...
            // console.log('not contract', this.isContract, res)
            if (res[1].data.state == 200) {
...
.find-page {
  ...
  .home-head {
    ...
    background: #1bb28c;
    background-image: url('https://uss.img.lezhulife.com/xcximages/bg_select_shop.png');
